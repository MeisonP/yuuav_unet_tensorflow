...
import numpy as np
import layers
from config import class_num, keep_prob
...
def net(input_):
    ...
    net['dropout4'] = layers.dropout(net['conv4_2'], keep_prob, name='dropout4')

    # block 5
    net['conv5_1'] = layers.conv_relu(net['pool4]'], 3, 1024, "conv5_1")
    net['conv5_2'] = layers.conv_relu(net['conv5_1'], 3, 1024, "conv5_2")
    net['dropout5']=layers.dropout(net['conv5_2'], keep_prob, name='dropout5')

    # #############deconv
    # block 6
    net['upsample6'] = layers.deconv(net['dropout5'], 1024, "upsample6")
    ...
    # block 7
    net['upsample7'] = layers.deconv(net['dropout5'], 512, "upsample7")
    net['concat7'] = tf.concat([net['upsampl e7'], net['conv3_2']], axis=3, name='concat7')
    ...
    # block 8
    net['upsample8'] = layers.deconv(net['dropout7'], 256, "upsample8")
    ...
    # block 9
    net['upsample9'] = layers.deconv(net['dropout8'], 128, "upsample9")
    ...
    # block 10
    net['conv10']=tf.nn.conv2d(net['conv9_2'],filter=tf.Variable(np.zeros([1,1,64,class_num]), tf.float32),
                               strides=[1,1], padding="SAME", name='conv10')
    # logits output
